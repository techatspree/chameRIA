...
java -d32 -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=9876 -Dchameleon.log.level=INFO -Dcom.trolltech.qt.library-path-override=qt/lib -XstartOnFirstThread de.akquinet.chameria.launcher.Launcher  --config=conf/chameleon.properties --deploy=deploy --debug "$@"
